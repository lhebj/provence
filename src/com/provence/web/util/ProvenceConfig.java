...
public class ProvenceConfig {
	...
	static {
		try {
			File file = new File(System.getProperty("catalina.home") + "/conf/ProvenceConfig.properties");
			if (file.exists()) {
				 ...
				 * use default config
				 */
				in = ProvenceConfig.class.getClassLoader().getResourceAsStream("ProvenceConfig.properties");
			}
			config.load(in);
